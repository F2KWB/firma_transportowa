...
export default function Offer() {
  ...
  return (
    /* Zmiana: w-full na mobile, max-w-7xl i rounded na desktopie */
    <section id="oferta" className="w-full max-w-7xl mx-auto bg-white md:rounded-[3rem] md:my-10 shadow-sm overflow-hidden">
      <div className="py-20 px-6 md:px-12">
        <div className="mb-16 text-center">
          <h2 className="text-sm uppercase tracking-[0.3em] text-gray-400 font-bold mb-4">Nasze Usługi</h2>
          <h3 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter">Oferujemy</h3>
        </div>

        {/* Grid: gap-0 na mobile (kafelki stykają się liniami), gap-6 na desktopie */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 md:gap-6">
          {offers.map((item, index) => (
            <div 
              key={index} 
              className="group p-10 border-b border-gray-100 md:border md:rounded-3xl hover:bg-black transition-all duration-300 text-center md:text-left flex flex-col items-center md:items-start"
            >
              <span className="inline-block px-3 py-1 bg-gray-100 group-hover:bg-white/20 text-[10px] font-bold rounded-full mb-6 group-hover:text-white transition-colors">
                {item.tag}
              </span>
              <h4 className="text-2xl font-bold uppercase italic mb-4 group-hover:text-white transition-colors">
                {item.title}
              </h4>
              <p className="text-gray-500 text-sm leading-relaxed group-hover:text-gray-300 transition-colors">
                {item.desc}
